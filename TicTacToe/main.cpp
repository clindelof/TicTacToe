...
{ 
	...
	int playersRunning = 0;
	std::mutex playersRunningMutex;
	std::condition_variable playersRunningCondition;

	std::atomic<bool> startPlay;
	std::mutex startPlayMutex;
	std::condition_variable startPlayCondition;
};
...
void LogSync(LogSyncOperation operationToPerform)
{
	...
	else if (operationToPerform == LogSyncOperation::Unlock) {
		printLock.unlock();
	}
}
	...
	{ 
		...
		switch (currentGame->currentGameState)
		{
			case GameState::StillPlaying:
				...
				currentGame->gameCondition.wait(*(currentGame->gameUniqueLock), [currentGame, currentPlayer] { return (currentGame->currentTurn == currentPlayer->type); });
...
void PlayerThreadEntrypoint(Player *currentPlayer)
{
	...
	currentPlayer->playerPool->playersRunningMutex.lock();
	currentPlayer->playerPool->playersRunning++;
	currentPlayer->playerPool->playersRunningMutex.unlock();

	currentPlayer->playerPool->playersRunningCondition.notify_all();

	std::unique_lock<std::mutex> lock(currentPlayer->playerPool->startPlayMutex);
	currentPlayer->playerPool->startPlayCondition.wait(lock, [currentPlayer] { return currentPlayer->playerPool->startPlay.load(); });
	lock.unlock();
...
int main(int argc, char **argv)
{
	...
	///////////////////////////////////////////////////////////////////////////////////
	poolOfPlayers.playersRunning = 0;
	poolOfPlayers.startPlay.store(false);
	...
	///////////////////////////////////////////////////////////////////////////////////
	std::unique_lock<std::mutex> waitForPlayers( poolOfPlayers.playersRunningMutex );
	poolOfPlayers.playersRunningCondition.wait(waitForPlayers, [&poolOfPlayers, totalPlayerCount] {return poolOfPlayers.playersRunning == totalPlayerCount;});

	///////////////////////////////////////////////////////////////////////////////////
	// TODO:: Notify all waiting threads that they can start playing.
	///////////////////////////////////////////////////////////////////////////////////
	poolOfPlayers.startPlayMutex.lock();
	poolOfPlayers.startPlay.store(true);
	poolOfPlayers.startPlayMutex.unlock();

	poolOfPlayers.startPlayCondition.notify_all();

	///////////////////////////////////////////////////////////////////////////////////
	// TODO:: Wait for all detached player threads to complete.
	///////////////////////////////////////////////////////////////////////////////////
	poolOfPlayers.playersRunningCondition.wait(waitForPlayers, [&poolOfPlayers] {return poolOfPlayers.playersRunning == 0; });
